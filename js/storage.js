...
const getRecentIngredients=()=>{try{return JSON.parse(localStorage.getItem(KEYS.recentIngredients)||'[]');}catch(e){return[];}};
const saveRecentIngredients=r=>localStorage.setItem(KEYS.recentIngredients,JSON.stringify(r));
function addToRecentIngredients(ingredient){
  ...
  list.unshift({name:ingredient.name,kcal:ingredient.kcal,protein:ingredient.protein,carbs:ingredient.carbs,fat:ingredient.fat,fibre:ingredient.fibre??0,lastUsed:Date.now()});
